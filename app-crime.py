...
df = pd.read_excel('DS_DV_Final.xlsx', sheet_name='Folha1')
df1 = pd.read_excel('DS_DV_onlycrime_Final.xlsx')
df2 = pd.read_excel('DS_DV_Final.xlsx', sheet_name='scatterplot')
...
sidebar = html.Div(
    [
        html.H3('Choose the crime, year and region you want to explore'),
        html.Br(),
        html.Label('Crime categories', style={'color': '#ffffff', 'font-family': 'Times New Roman'}),
        dropdown_crime,
        html.Br(),
        html.Label('Year', style={'color': '#ffffff', 'font-family': 'Times New Roman'}),
        slider_year,
        html.Br(),
        dropdown_tabs,
        # MAP
...
content_second_row = dbc.Row(
    [
        dbc.Col(
            dcc.Graph(id='graph_1', className='graph'), md=12
        )
    ]
)
...
content_third_row = dbc.Row(
    [
        dbc.Col(
            dcc.Graph(id='graph_2', className='graph'), md=12
        )
    ]
)
...
content_fourth_row = dbc.Row(
    [
        dbc.Col(
            dcc.Graph(id='graph_3', className='graph'), md=12
        )
    ]
)
...
content_fifth_row = dbc.Row(
    [
        dbc.Col(
            dcc.Graph(id='graph_4', className='graph'), md=12
        )
    ]
)
...
content = html.Div(
    [
        ...
        html.Br(),
        html.H6([
            'Authors: Carolina Rodrigues(20211298), Elena Nozal(20210989), Elsa Camuamba(20210992) and '
            'Manuel Pedro(20210999)',
            html.Br(), 'Source: Instituto Nacional de Estatística (INE)'], style={'font-size': '15px'}),
    ],
    style=CONTENT_STYLE
)
    ...
    if tab == "I":
        choropleth = px.choropleth_mapbox(ds[ds['NUTS'] == "I"], geojson=NUTS1, locations='ID', color=crime,
                                          ...
                                          labels={'Crimes Overall total (‰)': 'Rate',
                                                  ...
                                                  'Driving with a blood alcohol equal or above 1,2 g/l (‰)': 'Rate',
...
def update_graph1(clickData, crime):
    ...
    scatter_layout = dict(xaxis=dict(title='Year'),
                          ...
                          title='How have crime rates evolved over the years?<br><sup>  • Select a crime category '
                                'and/or a region to learn more! </sup>',
                          plot_bgcolor='rgba(0,0,0,0)',
                          colorway=px.colors.diverging.RdBu,
                          )
...
def update_graph2(year, clickData):
    ...
    barchart = px.bar(
        ...
        color_discrete_sequence=['#d1e5f0', '#67a9cf', '#2166ac'],
        barmode='group',
        title='How does a given crime category change over time?<br><sup> • '
              'Select a region or a year to find out! </sup>',
    )
...
def update_graph3(year, crime):
    ...
    scatterplot = px.scatter(
        ...
        hover_data=['Location'],
        color='NUTS II',
        title='Is there a relationship between a given crime category and house prices?<br><sup> • '
              'Select a crime category and/or a year to find out! </sup>',
        color_discrete_sequence=['#67001f', '#b2182b', '#d6604d', '#d1e5f0', '#67a9cf', '#2166ac', '#053061']
    )
...
def update_graph4(selected_year):
    ...
    fig = px.sunburst(sunburst_data, path=['NUTS I', 'NUTS II', 'NUTS III'],
                      ...
                      color_continuous_scale='RdBu',
                      color_continuous_midpoint=np.average(sunburst_data['Median value per m2 of dwellings sales (€)'],
                                                           weights=sunburst_data['Crimes Overall total (‰)']))
    fig.update_traces(
        go.Sunburst(
    ...
    fig.update_layout(dict(title=dict(text='Are high house prices synonymous with low crime regions?<br><sup> • '
                                           'Select a year and click on the figure to learn more! </sup>'),
